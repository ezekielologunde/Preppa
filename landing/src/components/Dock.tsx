...
export function Dock() {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-[1200px] mx-auto flex items-center justify-between gap-3 h-[54px] px-2.5 rounded-full bg-white/75 backdrop-blur-md border border-line text-ink-2 shadow-[0_10px_30px_rgba(0,0,0,.08)]">
          <div className="flex items-center gap-1">
            <button
              type="button"
              onClick={() => setLearnOpen(true)}
              className="h-10 px-3.5 rounded-full text-[13px] font-semibold hover:bg-ink/5 hover:text-ink transition-colors flex items-center gap-1.5"
            >
              <Icon name="chat" size={14} /> Learn
            </button>
            <a
              href="https://help.preppa.live/legal/terms"
              className="hidden sm:flex h-10 px-3.5 rounded-full text-[13px] font-semibold hover:bg-ink/5 hover:text-ink transition-colors items-center gap-1.5"
            >
              <Icon name="shield" size={14} /> Safety &amp; Legal
            </a>
          </div>

          <span className="hidden lg:block text-[12.5px] font-medium text-ink-soft truncate">
          ...
          <div className="flex items-center gap-1">
            <a
              ...
              className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-ink/5 hover:text-ink transition-colors"
            >
