-- ============================================================================
-- 0012 — Fix create_order: `min(m.prepper_id)` fails because Postgres has no
-- min() aggregate for uuid (42883). Since we already reject multi-prepper carts,
-- just pick the single prepper with a plain LIMIT 1 instead of an aggregate.
-- ============================================================================
...
begin
  ...
  -- Exactly one prepper at this point — grab it directly (no uuid aggregate).
